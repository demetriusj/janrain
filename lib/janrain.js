...
function janrain(EngageAPIKey) {
	return {
		...
		activity: function(identifier, activity, truncate, location, callback) {
      // JSON Stringify the Activity object as the 'queryString' lib's, 'stringify' method (which escapes POST data,
      //   does not handle subobjects.
      //  Without this step, no activty is sent to JanRain, and it returns with the error "activity is not a valid JSON object".
